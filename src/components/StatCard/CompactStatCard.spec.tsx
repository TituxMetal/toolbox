...
describe('CompactStatCard', () => {
  ...
  describe('Component Export', () => {
    it('exports CompactStatCard component as memoized component', () => {
      expect(typeof CompactStatCard).toBe('object')
      expect(CompactStatCard).toBeTruthy()
    })
  })

  describe('Basic Rendering', () => {
    it('renders element with children', () => {
      const element = createElement(CompactStatCard, {
        children: 'Test content'
      })
      expect(element.type).toBe(CompactStatCard)
      expect(element.props.children).toBe('Test content')
    })

    it('renders with default props', () => {
      const element = createElement(CompactStatCard, { children: 'Default' })
  ...
  describe('Edge Cases', () => {
    it('handles empty children', () => {
      const element = createElement(CompactStatCard, { children: '' })
      expect(element.props.children).toBe('')
    })

    it('handles null children', () => {
      const element = createElement(CompactStatCard, { children: null })
      expect(element.props.children).toBe(null)
    })

    it('handles number children', () => {
      const element = createElement(CompactStatCard, { children: 42 })
      expect(element.props.children).toBe(42)
    })

    it('handles React element children', () => {
      const child = createElement('span', { children: 'Nested' })
      const element = createElement(CompactStatCard, { children: child })
